...
namespace ConsoleApp16
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (UserColor == "red")
            {
                GetBucketsAndCost(area, 100, 25);
            }
            else if (UserColor == "blue")
            {
                GetBucketsAndCost(area, 120, 28);
            }
            else if (UserColor == "green")
            {
                GetBucketsAndCost(area, 90, 33);
            }
            else if (UserColor == "yellow")
            {
                GetBucketsAndCost(area, 70, 22);
            }

            Console.ReadLine();
        }
            static void GetBucketsAndCost(double area, double SqFt, decimal Price)
            {
                double Buckets = Math.Ceiling(area / SqFt);
                Console.WriteLine("You need " + Buckets + " buckets of paint.");
                decimal Cost = Convert.ToDecimal(Buckets) * Price;
